...
for c in top_countries_40:
    ...
    G.add_node(
        c, node_type="country", total_ied=float(total) if pd.notna(total) else 0.0
    )
...
for _, row in edge_filtered.iterrows():
    if row["IED"] > 0:
        amount = float(row["IED"])
        # NetworkX interpreta el atributo usado por los caminos mínimos como
        # distancia. Una relación con mayor IED debe representar mayor cercanía,
        # por eso se utiliza la inversa del monto como distancia económica.
        G.add_edge(
            row["País_origen"],
            row["Estado"],
            weight=amount,
...
print(f"  OK Red: {G.number_of_nodes()} nodos, {G.number_of_edges()} aristas")
...
print(f"  OK {n_comm} comunidades, modularidad = {modularity:.4f}")
...
print("  OK network_nodes guardado")
...
print("  OK network_edges guardado")
...
for comm_id in sorted(set(partition.values())):
    ...
    community_profiles[comm_id] = {
        "id": comm_id,
        "countries": countries_in,
        "states": states_in,
        "n_members": len(members),
        "total_ied_internal": round(sum(w for _, _, w in internal), 2),
        "top_country": sorted(
            countries_in, key=lambda c: G.nodes[c].get("total_ied", 0), reverse=True
        )[:3],
        "top_state": sorted(
            states_in, key=lambda s: G.nodes[s].get("total_ied", 0), reverse=True
        )[:3],
    }
...
graph_stats = {
    ...
    "shortest_path_distance": "inverse_ied",
    "top_state_hubs": df_nodes[df_nodes["node_type"] == "state"]
    .nlargest(5, "hub_score")[["node", "hub_score"]]
    .to_dict("records"),
    "top_country_hubs": df_nodes[df_nodes["node_type"] == "country"]
    .nlargest(5, "hub_score")[["node", "hub_score"]]
    .to_dict("records"),
}
