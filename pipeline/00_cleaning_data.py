...
def a_formato_largo(df, id_vars, value_name="IED"):
    df_largo = pd.melt(
        df, id_vars=id_vars, var_name="Año_Trimestre", value_name=value_name
    )
    df_largo["Fecha"] = (
...
def jerarquizar_scian(df, col_origen):
    ...
    df["Subsector"] = df["Código"].apply(
        lambda x: None if "-" in str(x) or len(str(x)) < 3 else str(x)[:3]
    )
    df["Rama"] = df["Código"].apply(
        lambda x: None if "-" in str(x) or len(str(x)) < 4 else str(x)[:4]
    )
    return df
...
a1 = pd.read_excel(
    RAW_DATA_DIR / "2025_3T_Flujosportipodeinversion_actu__3_.xlsx",
    sheet_name=None,
    header=None,
)
...
df_out.to_csv(
    PROCESSED_DATA_DIR / "Investment_Types.csv", index=False, encoding="utf-8-sig"
)
print(f"    OK Investment_Types.csv — {len(df_out):,} filas")

hoja = limpiar_encabezados(a1["Por origen"], 2, 3, "Origen")
df_out = a_formato_largo(hoja, ["Origen"])
df_out.to_csv(
    PROCESSED_DATA_DIR / "Origin_of_Investment.csv", index=False, encoding="utf-8-sig"
)
print(f"    OK Origin_of_Investment.csv — {len(df_out):,} filas")
...
df_out.to_csv(
    PROCESSED_DATA_DIR / "Investment_by_Sector.csv", index=False, encoding="utf-8-sig"
)
print(f"    OK Investment_by_Sector.csv — {len(df_out):,} filas")

hoja = limpiar_encabezados(a1["Por entidad federativa"], 2, 3, "Entidad federativa")
df_out = a_formato_largo(hoja, ["Entidad federativa"])
df_out.to_csv(
    PROCESSED_DATA_DIR / "Investment_by_State.csv", index=False, encoding="utf-8-sig"
)
print(f"    OK Investment_by_State.csv — {len(df_out):,} filas")

# Archivo 2
print("\nProcesando archivo 2: Flujos por entidad federativa...")
a2 = pd.read_excel(
    RAW_DATA_DIR / "2025_3T_Flujosporentidadfederativa_actu__5__.xlsx",
    sheet_name=None,
    header=None,
)
print(f"  Hojas: {list(a2.keys())}")

hoja = limpiar_encabezados(
    a2["Tipo de inversión"], 2, 3, "Entidad Federativa / Tipo de Inversión "
)
hoja = agregar_columna_estado(hoja, "Entidad Federativa / Tipo de Inversión ")
hoja = hoja.rename(
    columns={"Entidad Federativa / Tipo de Inversión ": "Tipo de Inversión"}
)
df_out = a_formato_largo(hoja, ["Tipo de Inversión", "Estado"])
df_out.to_csv(
    PROCESSED_DATA_DIR / "Investment_Type_by_State.csv",
    index=False,
    encoding="utf-8-sig",
)
print(f"    OK Investment_Type_by_State.csv — {len(df_out):,} filas")
...
hoja = hoja.rename(
    columns={"Entidad Federativa / Origen": "Inversión por país en cada estado"}
)
df_out = a_formato_largo(hoja, ["Inversión por país en cada estado", "Estado"])
df_out.to_csv(
    PROCESSED_DATA_DIR / "Investment_from_the_Countries_by_State.csv",
    index=False,
    encoding="utf-8-sig",
)
print(f"    OK Investment_from_the_Countries_by_State.csv — {len(df_out):,} filas")

hoja = limpiar_encabezados(
    a2["Actividad económica"], 2, 4, "Entidad Federativa / Actividad Económica"
)
...
hoja.loc[mask_e, "Estado"] = hoja.loc[
    mask_e, "Entidad Federativa / Actividad Económica"
]
hoja["Estado"] = hoja["Estado"].replace(["", False], np.nan).ffill()
hoja.loc[mask_e, ["Entidad Federativa / Actividad Económica", "Concepto"]] = (
    "Total general por estado"
)
for _col in ["Nivel", "Sector", "Subsector", "Rama"]:
    hoja[_col] = hoja[_col].astype(object)
...
df_out = a_formato_largo(
    hoja, ["Concepto", "Estado", "Nivel", "Sector", "Subsector", "Rama"]
)
df_out.to_csv(
    PROCESSED_DATA_DIR / "Investment_by_Sector_from_the_States.csv",
    index=False,
    encoding="utf-8-sig",
)
print(f"    OK Investment_by_Sector_from_the_States.csv — {len(df_out):,} filas")

# Archivo 3
print("\nProcesando archivo 3: Flujos por origen...")
a3 = pd.read_excel(
    RAW_DATA_DIR / "2025_3T_Flujospororigen_actu__4_.xlsx", sheet_name=None, header=None
)
print(f"  Hojas: {list(a3.keys())}")

hoja = limpiar_encabezados(
    a3["Por tipo de inversión"], 2, 3, "Origen / Tipo de Inversión "
)
...
df_out.to_csv(
    PROCESSED_DATA_DIR / "Investment_Types_by_Country.csv",
    index=False,
    encoding="utf-8-sig",
)
print(f"    OK Investment_Types_by_Country.csv — {len(df_out):,} filas")

hoja = limpiar_encabezados(
    a3["Por entidad federativa"], 2, 4, "Origen / Entidad Federativa"
)
hoja, _ = agregar_columna_pais(hoja, "Origen / Entidad Federativa")
hoja = hoja.rename(
    columns={"Origen / Entidad Federativa": "Inversión en cada estado por país"}
)
df_out = a_formato_largo(hoja, ["Inversión en cada estado por país", "País"])
df_out.to_csv(
    PROCESSED_DATA_DIR / "Investment_by_State_from_the_Countries.csv",
    index=False,
    encoding="utf-8-sig",
)
print(f"    OK Investment_by_State_from_the_Countries.csv — {len(df_out):,} filas")
...
df_out = a_formato_largo(
    hoja, ["País de origen", "Concepto", "Nivel", "Sector", "Subsector", "Rama"]
)
df_out.to_csv(
    PROCESSED_DATA_DIR / "Investment_by_Sector_from_the_Countries.csv",
    index=False,
    encoding="utf-8-sig",
)
print(f"    OK Investment_by_Sector_from_the_Countries.csv — {len(df_out):,} filas")
