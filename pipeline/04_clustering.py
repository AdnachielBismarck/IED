...
# Agrupa los 32 estados en tipologías económicas usando KMeans validado.
# RUTAS: IED/pipeline/ a .parent.parent = IED/
...
print("=" * 70)
print("PASO 4 — CLUSTERING ESTRUCTURAL DE ESTADOS")
print("=" * 70)
...
state_nodes = df_nodes[df_nodes["node_type"] == "state"][
    ["node", "hub_score", "betweenness", "community"]
...
annual = (
    df_states.groupby("Entidad federativa")["IED"]
    .agg(total_ied_alltime="sum")
    .reset_index()
    .rename(columns={"Entidad federativa": "estado"})
)
df_feat = df_feat.merge(annual, on="estado", how="left")

country_diversity = (
    df_cs_raw[df_cs_raw["IED"].notna() & (df_cs_raw["IED"] > 0)]
    .groupby("Estado")["País_origen"]
    .nunique()
    .reset_index()
    .rename(columns={"Estado": "estado", "País_origen": "n_investing_countries"})
)
df_feat = df_feat.merge(country_diversity, on="estado", how="left")

# La segmentación utiliza dimensiones conceptualmente distintas. Hub Score,
# volumen y conteos se conservan para describir los grupos, pero no entran en
# KMeans porque duplican la señal de escala y aislaban a Ciudad de México.
feature_cols = [
    "dependency_score",
...
profile_cols = feature_cols + [
    ...
    "betweenness",
    "n_active_countries",
    "n_investing_countries",
    "mean_ied",
]
df_feat_clean = df_feat[["estado"] + profile_cols].copy()
...
print(
    f"  PCA varianza explicada: {explained[0]:.1%} + {explained[1]:.1%} = {sum(explained):.1%}"
)

print("Evaluando KMeans (k=2 a k=6)...")
diagnostics = []
models = {}
for k in range(2, 7):
    candidate = KMeans(n_clusters=k, random_state=42, n_init=50)
    candidate_labels = candidate.fit_predict(X_scaled)
    counts = np.bincount(candidate_labels)
    row = {
        "k": k,
        "silhouette": float(silhouette_score(X_scaled, candidate_labels)),
        "calinski_harabasz": float(calinski_harabasz_score(X_scaled, candidate_labels)),
        "min_cluster_size": int(counts.min()),
    }
    diagnostics.append(row)
    models[k] = (candidate, candidate_labels)

# En una muestra de 32 estados, una tipología debe contener al menos tres
# observaciones para ser interpretable. Entre las soluciones válidas se elige
# la de mayor silhouette; si ninguna cumple, se usa el máximo global.
valid = [row for row in diagnostics if row["min_cluster_size"] >= 3]
selected = max(valid or diagnostics, key=lambda row: row["silhouette"])
selected_k = selected["k"]
km, labels = models[selected_k]
sil = selected["silhouette"]
distances = km.transform(X_scaled)
nearest = np.partition(distances, 1, axis=1)[:, :2]
confidence = 1 - (nearest[:, 0] / (nearest[:, 1] + 1e-9))
print(f"  Selección: k={selected_k} | Silhouette={sil:.3f}")

df_feat_clean = df_feat_clean.copy()
df_feat_clean["cluster_id"] = labels
df_feat_clean["cluster_confidence"] = confidence.round(4)
df_feat_clean["pca_x"] = X_pca[:, 0]
df_feat_clean["pca_y"] = X_pca[:, 1]

global_means = {c: float(np.nanmean(df_feat_clean[c])) for c in profile_cols}
...
for cid in sorted(set(labels)):
    ...
    mean_vals = subset[profile_cols].mean()
    ...
    high_div = (
        mean_vals["diversification_score"]
        > global_means["diversification_score"] * 1.10
    )
    low_div = (
        mean_vals["diversification_score"]
        < global_means["diversification_score"] * 0.90
    )
    ...
    if high_hub and high_div:
        ...
        diffs = {
            c: (mean_vals[c] - global_means[c]) / (global_means[c] + 1e-6)
            for c in profile_cols
            if global_means[c] > 0
        }
        top_feat = max(diffs, key=lambda k: abs(diffs[k]))
        feat_labels = {
            "nearshoring_score": "Nearshoring Activo",
            "dependency_score": "Alta Dependencia",
            "diversification_score": "Alta Diversificación",
            "stability_index": "Alta Estabilidad",
            "hub_score": "Hub Emergente",
            "n_investing_countries": "Amplia Base Inversora",
        }
...
print("  OK state_clusters guardado")
...
print("  OK cluster_summary.json guardado")

with open(PROC / "clustering_diagnostics.json", "w", encoding="utf-8") as f:
    json.dump(
        {
            "selected_k": selected_k,
            "selection_rule": "highest silhouette with minimum cluster size of 3",
            "pca_explained_variance": [round(float(v), 4) for v in explained],
            "candidates": [
                {
                    **row,
                    "silhouette": round(row["silhouette"], 4),
                    "calinski_harabasz": round(row["calinski_harabasz"], 2),
                }
