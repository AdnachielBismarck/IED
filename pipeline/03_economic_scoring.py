...
print("=" * 70)
print("PASO 3 — SCORING ECONÓMICO E INDICADORES ESTRUCTURALES")
print("=" * 70)
...
all_states = [
    s for s in df_states["Entidad federativa"].unique() if s not in ["Total general"]
]
...
for state in all_states:
    ...
    if len(sub) == 0:
        dep_scores.append(
            {
                "estado": state,
                "dependency_score": np.nan,
                "top_country": None,
                "top_country_share": np.nan,
                "n_active_countries": 0,
            }
        )
        continue
    ...
    dep_scores.append(
        {
            "estado": state,
            "dependency_score": round(score * 100, 2),
            "top_country": top_c,
            "top_country_share": round(top_sh * 100, 2),
            "n_active_countries": int((by_country > 0).sum()),
        }
    )
df_dep = pd.DataFrame(dep_scores)
print(f"  OK {len(df_dep)} estados")
...
type_data = df_types_s[
    df_types_s["IED"].notna() & ~df_types_s["Tipo de Inversión"].isin(excl_tipos)
].copy()
div_scores = []
for state in all_states:
    ...
    div_scores.append(
        {
            "estado": state,
            "diversification_score": round(score * 100, 2),
            "dominant_type": by_type.idxmax(),
            "n_investment_types": int(len(by_type)),
        }
    )
df_div = pd.DataFrame(div_scores)
print(f"  OK {len(df_div)} estados")
...
for state in all_states:
    ...
    if len(sub) == 0:
        obs_scores.append(
            {"estado": state, "observability_risk": np.nan, "pct_confidential": np.nan}
        )
        continue
    n_total = len(sub)
    ...
    obs_scores.append(
        {
            "estado": state,
            "observability_risk": round(pct, 2),
            "pct_confidential": round(pct, 2),
            "n_total_records": n_total,
            "n_confidential_records": int(n_conf),
        }
    )
df_obs = pd.DataFrame(obs_scores)
print(f"  OK {len(df_obs)} estados")
...
for state in all_states:
    ...
    if len(ied_vals) < 4:
        stab_scores.append({"estado": state, "stability_index": np.nan, "cv": np.nan})
        continue
    ...
    stab_scores.append(
        {
            "estado": state,
            "stability_index": round(100 / (1 + cv), 2),
            "cv": round(cv, 4),
            "mean_ied": round(mean_v, 2),
            "std_ied": round(std_v, 2),
        }
    )
df_stab = pd.DataFrame(stab_scores)
print(f"  OK {len(df_stab)} estados")
...
for state in all_states:
    ...
    us_data = df_cs_raw[
        (df_cs_raw["Estado"] == state)
        & (df_cs_raw["País_origen"] == "Estados Unidos de América")
        & (df_cs_raw["IED"] > 0)
    ]
    state_total = df_cs_raw[(df_cs_raw["Estado"] == state) & (df_cs_raw["IED"] > 0)][
        "IED"
    ].sum()
    us_total = us_data["IED"].sum()
    us_share = us_total / state_total if state_total > 0 else 0
    positive = sub[sub["IED"] > 0]
    recent_share = positive[positive["Year"] >= 2021]["IED"].sum() / (
        positive["IED"].sum() + 1e-6
    )
    geo_bonus = 0.2 if state in border_states else (0.1 if state in bajio_states else 0)
    empirical_score = (
        0.45 * np.clip(growth_ratio, 0, 3) / 3
        + 0.35 * np.clip(us_share, 0, 1)
        + 0.20 * np.clip(recent_share, 0, 1)
    )
    strategic_score = 0.85 * empirical_score + 0.15 * geo_bonus / 0.2
    ns_scores.append(
        {
            "estado": state,
            "nearshoring_score": round(empirical_score * 100, 2),
            "nearshoring_strategic_score": round(strategic_score * 100, 2),
            "growth_ratio_2021_24": round(growth_ratio * 100, 2),
            "us_share_pct": round(us_share * 100, 2),
            "zone": nearshoring_zone.get(state, "centro/sur"),
        }
    )
df_ns = pd.DataFrame(ns_scores)
print(f"  OK {len(df_ns)} estados")
...
df_score = df_score.merge(
    df_stab[["estado", "stability_index", "cv", "mean_ied", "std_ied"]],
    on="estado",
    how="outer",
)
df_score = df_score.merge(
...
print(f"  OK state_scores guardado — {len(df_score)} estados")
...
for country in df_ct["Origen"].unique():
    ...
    country_conc.append(
        {
            "country": country,
            "concentration_hhi": round(h * 100, 2),
            "n_states_active": int((pos > 0).sum()),
            "top_state": pos.idxmax(),
            "total_ied": round(pos.sum(), 2),
        }
    )
df_cp = pd.DataFrame(country_conc).sort_values("total_ied", ascending=False)
_save(df_cp, "country_profiles.parquet")
print(f"  OK country_profiles — {len(df_cp)} países")
...
print(
    df_score.nlargest(5, "strategic_score")[
        [
            "estado",
            "strategic_score",
...
print(
    df_score.nlargest(5, "nearshoring_score")[
        ["estado", "nearshoring_score", "zone", "us_share_pct", "growth_ratio_2021_24"]
    ].to_string(index=False)
)
