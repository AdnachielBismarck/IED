...
st.set_page_config(
    page_title="Perfiles estatales · IED México",
    layout="wide",
...
state_nodes = nodes[nodes["node_type"] == "state"][
...
render_header(
    "Perfiles estatales",
    "Diagnóstico comparativo de las 32 entidades federativas",
    "Consulta estructura, dependencia, diversificación, estabilidad y posición relativa de cada estado.",
)
...
with tab_matrix:
    ...
    fig_m.update_layout(
        **PLOT_LAYOUT,
        ...
        yaxis=dict(
            **YAXIS_BASE, title="Diversificación (score)", tickfont=dict(size=10)
        ),
...
with tab_profile:
    ...
    for col_k, val, label, inv in kpi_items:
        ...
        with col_k:
            st.markdown(
                f"""<div class="kpi-card" style="border-top-color:{c};">
    ...
    with col_r:
        ...
        fig_r.update_layout(
            ...
            title_font=dict(
                family="IBM Plex Sans, sans-serif", color="#e6edf3", size=13
            ),
            height=320,
...
with tab_compare:
    ...
    with cmp1:
        ...
        default_cmp = (
            "Ciudad de México"
            if "Ciudad de México" in all_states_cmp
            else all_states_cmp[0]
        )
...
st.download_button(
    "Descargar perfiles estatales",
    data=scores.sort_values("strategic_score", ascending=False)
    .to_csv(index=False)
    .encode("utf-8-sig"),
    file_name="perfiles_estatales_ied.csv",
    mime="text/csv",
)
render_provenance()
