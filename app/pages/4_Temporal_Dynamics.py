...
st.set_page_config(
    page_title="Dinámica temporal · IED México",
    layout="wide",
...
pre_avg = float(
    annual_total[(annual_total.index >= 2015) & (annual_total.index <= 2019)].mean()
)
...
render_header(
    "Dinámica temporal",
    "Evolución de la IED y señales posteriores a 2021",
    "Compara trimestres homogéneos, composición por tipo y cambios recientes por país y estado.",
)
...
tab1, tab2, tab3 = st.tabs(
    ["Evolución Temporal", "Señal Nearshoring", "Dinámica por País"]
)
...
with tab1:
    ...
    with fcol2:
        if modo_serie == "Comparar por trimestre":
            ...
            q_filtro = None
            st.caption(
                "Muestra todos los trimestres como serie continua con media móvil de 4 períodos."
            )
    ...
    if modo_serie == "Serie completa":
        ...
        fig_n.update_layout(
            **PLOT_LAYOUT,
            title=f"IED trimestral {PERIOD_LABEL}: la media móvil revela la tendencia estructural",
            height=400,
            xaxis=dict(**XAXIS_BASE, title="Trimestre", tickfont=dict(size=10)),
            yaxis=dict(
                **YAXIS_BASE, title="IED (USD Millones)", tickfont=dict(size=10)
            ),
            legend=LEGEND_H,
            margin=MARGIN_STD,
        )
        ...
        st.markdown(
            f"<p class='method-note'>Serie completa {PERIOD_LABEL} · Cada barra = un trimestre · "
        ...
        colors_bar = ["#3fb950" if (pd.isna(v) or v >= 0) else "#f85149" for v in delta]
...
with tab2:
    ...
    sd = (
        cs_raw[
            (cs_raw["Year"] >= 2022) & (cs_raw["IED"].notna()) & (cs_raw["IED"] > 50)
        ]
        .groupby(["País_origen", "Estado"])["IED"]
        .sum()
        .reset_index()
    )
    ...
    if len(sf) > 0:
        ...
        fig_sk = go.Figure(
            go.Sankey(
                ...
                link=dict(
                    ...
                    hovertemplate="<b>%{source.label} a %{target.label}</b><br>$%{value:,.0f} M<extra></extra>",
                ),
            )
        )
        fig_sk.update_layout(
            title="Flujos IED acumulados: Países de origen a Estados receptores (2022–2024)",
            template="plotly_dark",
            height=520,
            paper_bgcolor="#161b22",
        ...
        st.info(
            "No hay flujos suficientes para construir el diagrama con los datos disponibles."
        )
    ...
    with n2:
        ...
        fig_gr.update_layout(
            **PLOT_LAYOUT,
            ...
            xaxis=dict(
                **XAXIS_BASE,
                title="Crecimiento IED 2021–2024 (%)",
    ...
    fig_pp.update_layout(
        **PLOT_LAYOUT,
        ...
        yaxis=dict(
            **YAXIS_BASE,
            title="IED Promedio Anual (USD Millones)",
            tickfont=dict(size=10),
        ),
        legend=dict(**LEGEND_H),
...
st.download_button(
    "Descargar serie trimestral nacional",
    data=nq.to_csv(index=False).encode("utf-8-sig"),
    file_name="serie_trimestral_ied.csv",
    mime="text/csv",
)
render_provenance()
